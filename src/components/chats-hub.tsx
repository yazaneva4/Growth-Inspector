        ...
        <div className="flex-1 overflow-y-auto p-2">
          {visibleConversations.length === 0 ? (
            ...
            <button key={conversation.id} onClick={() => setSelectedId(conversation.id)} className={`mb-1 flex w-full items-center gap-3 rounded-xl p-3 text-left ${selectedId === conversation.id ? "bg-emerald-50" : "hover:bg-slate-50"}`}>
              <Avatar member={otherMember(conversation)} label={conversationName(conversation)} onlineUserIds={onlineUserIds} />
        ...
        {selected ? (
          <>
            <header className="flex items-center gap-3 border-b border-slate-200 px-4 py-3">
              ...
              <Avatar member={otherMember(selected)} label={conversationName(selected)} onlineUserIds={onlineUserIds} />
